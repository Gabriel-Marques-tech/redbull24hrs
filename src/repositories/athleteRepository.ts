...
export const athleteRepository = {
	async create(name: string, gender: string, cpf: string | null, team_id: number, email?: string | null) {
		const result = await pool.query(
			`INSERT INTO athletes (name, gender, cpf, team_id, email) VALUES ($1, $2, $3, $4, $5) RETURNING *`,
			[name, gender, cpf ?? null, team_id, email ?? null]
		);
	...
	async update(id: number, fields: { name?: string; gender?: string; cpf?: string | null; email?: string | null }) {
