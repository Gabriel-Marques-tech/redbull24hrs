...
A coluna **Status** reflete o estado de implementação na WebAPI (seção 3.7): *Implementado* para os endpoints já operantes entre os 38 disponíveis na documentação navegável; *Planejado (sprint 4)* para os cinco endpoints com contrato já definido (método, path e RN governante), porém ainda não implementados, conforme detalhado no Quadro 31 da seção 3.9; e *Frontend* para os RF cuja execução é responsabilidade da interface, consumindo um endpoint de leitura já existente no backend.
...
| RF003 | RN17, RN28                          | `GET /teams/:teamId/validation`                                                                                                                  | GET                                    | Planejado (sprint 4)            |
...
| RF020 | RN09                                | `GET /metrics/events/:id/teams`, `GET /metrics/events/:id/dashboard`                                                                             | GET                                    | Implementado                    |
| RF021 | RN11                                | `GET /metrics/events/:id/dashboard`                                                                                                              | GET                                    | Implementado                    |
| RF022 | RN13                                | `GET /audit/history`                                                                                                                             | GET                                    | Implementado                    |
| RF023 | RN24                                | `PATCH /teams/:teamId/athletes/:id`                                                                                                              | PATCH                                  | Implementado                    |
| RF024 | RN23                                | `GET /audit/logs`                                                                                                                                | GET                                    | Planejado (sprint 4)            |
| RF025 | RN27                                | `POST /audit/shifts/:id/checkpoints`                                                                                                             | POST                                   | Implementado                    |
| RF026 | RN27                                | `POST /audit/sync`                                                                                                                               | POST                                   | Planejado (sprint 4)            |
...
| RF031 | —                                   | `PATCH /audit/checkpoints/:id`                                                                                                                   | PATCH                                  | Planejado (sprint 4)            |
...
| RF035 | —                                   | `GET /metrics/events/:id/athletes`                                                                                                               | GET                                    | Implementado                    |
| RF036 | —                                   | `GET /metrics/athletes/:id/shifts`                                                                                                               | GET                                    | Implementado                    |
| RF037 | RN10                                | `GET /metrics/athletes/:id/snapshots`                                                                                                            | GET                                    | Implementado                    |
| RF038 | RN12                                | `GET /metrics/events/:id/dashboard`                                                                                                              | GET                                    | Implementado                    |
| RF039 | RN12                                | `GET /audit/alerts`                                                                                                                              | GET                                    | Implementado                    |
| RF040 | RN14                                | `GET /metrics/events/:id/dashboard`                                                                                                              | GET                                    | Frontend (Modo TV)              |
...
| RF047 | RN26                                | `GET /export/events/:id/shifts`                                                                                                                  | GET                                    | Implementado                    |
| RF048 | RN26                                | `GET /export/events/:id/checkpoints`                                                                                                             | GET                                    | Implementado                    |
| RF049 | —                                   | `GET /metrics/athletes/:id/performance`                                                                                                          | GET                                    | Implementado                    |
| RF050 | RN36                                | `GET /metrics/athletes/:id/share`                                                                                                                | GET                                    | Planejado (sprint 4)            |
| RF051 | RN18, RN29, RN37                    | `POST /events`, `PATCH /events/:id`                                                                                                              | POST / PATCH                           | Implementado                    |
...
Observa-se que os endpoints de cada RF pertencem aos dez fluxos consolidados na documentação da WebAPI (seção 3.7): Autenticação, Eventos, Esteiras, Equipes, Atletas, Turnos, Histórico, Alertas, Métricas e Exportação. Os cinco endpoints classificados como *Planejado (sprint 4)* — `GET /teams/:teamId/validation` (RF003), `GET /audit/logs` (RF024), `POST /audit/sync` (RF026), `PATCH /audit/checkpoints/:id` (RF031) e `GET /metrics/athletes/:id/share` (RF050) — já têm contrato definido e serão implementados de forma incremental, sem alterar os endpoints existentes, conforme o Quadro 31 (seção 3.9). Os RF marcados como *Frontend* (RF029, RF030, RF034 e RF040) não demandam um endpoint próprio: sua execução ocorre na camada de interface, reutilizando endpoints de leitura já operantes no backend (notificação visual e sonora de alertas a partir de `GET /audit/alerts`, *hot swap* a partir de `POST /audit/shifts/start` e Modo TV a partir de `GET /metrics/events/:id/dashboard`).
...
Dos endpoints mapeados nesta matriz, a maior parte já está implementada e operante entre os 38 endpoints da WebAPI (seção 3.7). Os cinco endpoints marcados com *(sprint 4)* possuem contrato definido — método, path e RN governante —, mas sua implementação está planejada para a sprint 4, conforme detalhado no Quadro 31 ao final desta seção. Essa marcação mantém a coerência com a coluna *Status* da Matriz RF → RN → Endpoint (seção 3.1.4).
...
| LA, BG | RF003 | RN17, RN28 | `GET /teams/:teamId/validation` *(sprint 4)* | Tela de Registro Pré-Evento → Cadastro de Equipe (listagem) | CT-006: Tentar iniciar evento com equipe com menos de 16 corredores → bloqueio com mensagem indicando o número faltante; CT-007: Ambas as equipes com 16 corredores → início permitido |
...
| BG | RF024 | RN23 | `GET /audit/logs` *(sprint 4)* | Tela de Acompanhamento (log de auditoria) | CT-045: Auditor edita quilometragem de checkpoint → log registra usuário, campo, valor anterior, valor novo e timestamp; CT-046: Admin consulta log → todas as edições do registro em ordem cronológica |
...
| LA | RF026 | RN27 | `POST /audit/sync` *(sprint 4)* | Tela de Acompanhamento de Esteiras (indicador de sync) | CT-049: Conexão restabelecida → sincronização automática de todos os registros pendentes; CT-050: Dados sincronizados consultados no servidor → cada registro aparece exatamente uma vez |
...
| LA | RF031 | — | `PATCH /audit/checkpoints/:id` *(sprint 4)* | Tela de Inconsistência Detectada | CT-059: Auditor corrige valor para dado consistente → confirmação desbloqueada e dado marcado como revisado; CT-060: Auditor mantém valor original com justificativa → persistido com flag "revisado manualmente" e justificativa associada |
...
| NR | RF050 | RN36 | `GET /metrics/athletes/:id/share` *(sprint 4)* | Tela de Desempenho Final | CT-090: Acionar "Compartilhar" → URL única e pública gerada; CT-091: Link acessado por usuário não cadastrado → exibe apenas dados de desempenho do corredor sem acesso a outras funcionalidades |
...
A maior parte dos endpoints da RTM já está implementada e operante na WebAPI (ver seção 3.7). Os endpoints listados abaixo estão **planejados para a sprint 4**, completando a cobertura dos RF de validação pré-evento, auditoria retroativa, sincronização offline, revisão de inconsistências e compartilhamento público. O contrato (método, path e RN governante) já está definido para que a implementação seja uma evolução incremental, sem alterar os endpoints existentes. Esses cinco endpoints correspondem exatamente aos RF marcados como *Planejado (sprint 4)* na Matriz RF → RN → Endpoint (seção 3.1.4).
...
| RF031 | `PATCH /audit/checkpoints/:id` | — | Corrigir o valor de um checkpoint inconsistente, desbloqueando a confirmação após revisão. **Plano:** novo método no `shiftService` que valida o novo valor contra os checkpoints vizinhos (regra de edição análoga à RN24), grava a alteração na trilha de auditoria (análoga à RN23) e marca o registro como revisado. |
