...
| **Critério de aceite 1** | CR1: deve ser possível finalizar corrida apenas quando houver corrida ativa, com inserção manual do valor final de km.<br>**Validação:** verificar se o campo de finalização está disponível somente com corrida ativa.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| **Teste de aceitação 1** | Com corrida ativa, inserir valor final de km e finalizar; verificar data/horário automáticos e persistência.<br>**Esperado:** corrida finalizada e dados persistidos corretamente.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| USAB — Usabilidade       | Prevenção de erro humano                                                                       | RF018, RF019                            | O sistema deve apresentar alertas claros e compreensíveis para inconsistências.                    | Usuário deve entender o alerta em até 2 segundos.                                                     | Feedback visual com cores, ícones e mensagens diretas.                        |
...
| CONF — Confiabilidade    | Possível perda de conexão                                                                      | RF016                                   | O sistema deve operar offline e sincronizar dados posteriormente.                                  | Perda de dados ≤ 5 minutos (RPO).                                                                     | Persistência local (IndexedDB) com sincronização automática.                  |
| CONF — Confiabilidade    | Garantia de integridade de dados                                                               | RF010, RF011                            | O sistema deve impedir persistência de dados inválidos.                                             | 100% dos dados inválidos rejeitados.                                                                  | Validação de regras antes do salvamento.                                     |
| CONF — Confiabilidade    | Inserção incorreta de dados (erro humano)                                                      | RF018, RF019, RF020                     | O sistema deve detectar e bloquear inconsistências antes da persistência.                          | 100% das inconsistências detectadas antes da confirmação.                                             | Validação em tempo real com bloqueio de ação.                                |
| CONF — Confiabilidade    | Continuidade da operação                                                                       | RF007, RF009                            | O sistema deve garantir consistência de estado entre turnos.                                        | Nenhum corredor pode ter mais de um turno ativo simultaneamente.                                     | Controle de estado no backend.                                               |
...
| DES — Desempenho         | Feedback imediato ao usuário                                                                   | RF018, RF019                            | O sistema deve apresentar alertas em tempo real.                                                    | Tempo de exibição de alerta < 100ms.                                                                  | Processamento local + validação leve.                                        |
| DES — Desempenho         | Visualização do placar                                                                         | RF013                                   | O sistema deve atualizar o dashboard em tempo quase real.                                           | Atualização ≤ 10 segundos sem reload.                                                                 | Uso de polling ou WebSocket.                                                 |
...
| SEG — Segurança          | Controle de acesso                                                                            | RF017                                   | O sistema deve exigir autenticação para ações críticas.                                             | 100% das ações críticas autenticadas.                                                                 | Login com sessão/token.                                                      |
| SEG — Segurança          | Rastreabilidade                                                                               | RF015                                   | O sistema deve registrar todas as alterações realizadas.                                            | 100% das edições com log completo (antes/depois/usuário).                                            | Logs de auditoria no backend.                                                |
| SEG — Segurança          | Correção de inconsistências                                                                   | RF019, RF020                            | O sistema deve registrar quem corrigiu inconsistências.                                             | Toda correção deve conter usuário e timestamp.                                                        | Auditoria detalhada.                                                         |
| SEG — Segurança          | Integridade contra manipulação                                                                | RF010, RF018                            | O sistema deve impedir alteração inválida de dados.                                                 | Nenhum valor inválido persistido.                                                                     | Validação e sanitização de inputs.                                           |
| SUP — Suportabilidade    | Evolução futura do sistema                                                                    | RF011, RF018, RF020                     | O sistema deve permitir fácil modificação das regras de negócio.                                    | Tempo de alteração de regra ≤ 1 dia.                                                                  | Arquitetura modular.                                                         |
| SUP — Suportabilidade    | Facilidade de manutenção                                                                      | RF007, RF010                            | O sistema deve ser facilmente testável e compreensível.                                             | Cobertura de testes ≥ 70%.                                                                            | Testes unitários e documentação.                                             |
| SUP — Suportabilidade    | Organização do código                                                                         | RF011                                   | O sistema deve separar lógica de cálculo da interface.                                              | Nenhuma lógica crítica no frontend.                                                                   | Camadas bem definidas (backend/frontend).                                   |
| CAP — Capacidade         | Múltiplos usuários simultâneos                                                                | RF013, RF021                            | O sistema deve suportar acessos simultâneos sem degradação.                                         | Até 20 conexões simultâneas.                                                                          | Infra dimensionada adequadamente.                                            |
| CAP — Capacidade         | Crescimento do uso                                                                            | RF029, RF030                            | O sistema deve suportar aumento de acessos às telas de resultado.                                   | Escalabilidade horizontal disponível.                                                                 | Arquitetura escalável.                                                       |
| CAP — Capacidade         | Volume de dados                                                                               | RF011, RF023                            | O sistema deve lidar com grande volume de registros.                                                | Processamento sem perda de performance perceptível.                                                   | Banco otimizado e indexado.                                                  |
| REST — Restrições Design | Ambiente instável                                                                             | RF016                                   | O sistema não deve depender de conexão constante.                                                   | Sistema funcional offline.                                                                            | Armazenamento local.                                                         |
| REST — Restrições Design | Infraestrutura variável                                                                       | RF007, RF008                            | O sistema deve funcionar em diferentes dispositivos.                                                | Compatível com tablets e navegadores modernos.                                                        | Design responsivo.                                                           |
| REST — Restrições Design | Independência tecnológica                                                                     | RF018                                   | O sistema deve operar sem dependência de APIs externas críticas.                                    | 0 dependência crítica externa.                                                                        | Processamento interno.                                                       |
| ORG — Organizacionais    | Alinhamento com evento                                                                        | RF021, RF025                            | O sistema deve seguir identidade visual da marca.                                                   | 100% aderência ao design guideline.                                                                   | Uso de design system oficial.                                                |
| ORG — Organizacionais    | Cronograma do evento                                                                          | RF013                                   | O sistema deve estar pronto antes do evento.                                                        | Entrega ≥ 1 mês antes.                                                                                | Planejamento e validação contínua.                                          |
| ORG — Organizacionais    | Experiência do atleta                                                                         | RF029, RF030                            | O sistema deve permitir visualização e compartilhamento de resultados.                              | 100% dos atletas conseguem acessar seus resultados.                                                   | Tela dedicada + geração de link/imagem.                                      |                                                            |
