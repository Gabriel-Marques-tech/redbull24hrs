...
Para trabalhos futuros, o grupo identificou três frentes principais de evolução. A primeira diz respeito ao OCR utilizado na leitura de documentos: hoje o sistema conta com APIs gratuitas de inteligência artificial para essa função, mas a migração para APIs pagas permitiria maior precisão e consistência nos resultados. A segunda envolve o envio de e-mails, que atualmente depende de uma API gratuita com limitações de entrega; a adoção de um serviço pago tornaria esse fluxo mais confiável. Por fim, o grupo pretende investir em uma assinatura de plataforma de deploy, o que daria acesso a mais recursos de infraestrutura, incluindo a possibilidade de definir um domínio próprio para a aplicação.
