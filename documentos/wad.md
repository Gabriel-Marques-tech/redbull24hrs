...
No contexto deste projeto, os requisitos funcionais servem como guia tanto para a equipe de desenvolvimento quanto para os organizadores do evento, assegurando que todas as necessidades operacionais, como o registro de quilometragem e o controle de revezamento entre corredores, sejam atendidas com precisão e confiabilidade.

| ID    | Descrição                                                                                                                                                                                                                                                        | Prioridade | Status    |
| ----- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- | --------- |
| RF001 | O sistema deve permitir que o Auditor registre o início de um turno, armazenando corredor, esteira, quilometragem inicial e timestamp automático do servidor, somente se o corredor não possuir turno em aberto e a esteira estiver com status "Livre". | Alta       | Planejado |
| RF002 | O sistema deve exibir um modal bloqueante a cada 5 minutos a partir do início do turno, impedindo qualquer interação até que a quilometragem atual seja inserida (valor ≥ último checkpoint registrado).                                                         | Alta       | Planejado |
| RF003 | O sistema deve permitir o registro manual de quilometragem a qualquer momento, gerando timestamp automático para garantir rastreabilidade.                                                                                                                       | Média      | Planejado |
| RF004 | O sistema deve permitir que o Auditor finalize o turno de um corredor, acionando o fluxo de encerramento e o cálculo automático de estatísticas da corrida.                                                                                                                | Alta       | Planejado |
| RF005 | O sistema deve permitir a inserção da quilometragem final com registro automático de timestamp, rejeitando valores inferiores ao último checkpoint registrado.                                                                                                   | Alta       | Planejado |
| RF006 | O sistema deve calcular automaticamente a distância percorrida (km_final − km_inicial), a duração do turno (timestamp_fim − timestamp_início) e a velocidade média (km/h), persistindo os dados vinculados ao respectivo turno.                                 | Alta       | Planejado |
| RF007 | O sistema deve permitir iniciar um novo corredor na mesma esteira com um único clique após o término do turno anterior, reutilizando os dados da equipe já cadastrados.                                                                                         | Média      | Planejado |
| RF008 | O sistema deve calcular automaticamente a quilometragem total acumulada por equipe, somando as distâncias percorridas individualmente por cada corredor.                                                                                                         | Alta       | Planejado |
| RF009 | O sistema deve gerar métricas individuais por corredor, incluindo distância total percorrida, média por turno e histórico de evolução por hora com snapshots a cada 60 minutos.                                                                                  | Média      | Planejado |
| RF010 | O sistema deve exibir um dashboard com placar e métricas atualizados automaticamente em intervalos de até 10 segundos, sem necessidade de recarregamento de página.                                                                                             | Alta       | Planejado |
| RF011 | O sistema deve exibir o status de cada esteira (Ocupada/Livre) e sugerir alternância entre elas para prevenir superaquecimento dos equipamentos.                                                                                                                 | Média      | Planejado |
| RF012 | O sistema deve exibir um histórico (log) completo de entradas, saídas e checkpoints, ordenado de forma decrescente por data e hora.                                                                                                                             | Alta       | Planejado |
| RF013 | O sistema deve disponibilizar um modo TV com fonte mínima de 48px, contraste em conformidade com WCAG AA, resolução 1920×1080, operável sem mouse e sem necessidade de login.                                                                                   | Média      | Planejado |
| RF014 | O sistema deve permitir o cadastro de exatamente duas equipes, cada uma com nome e identificador únicos, bloqueando o registro em caso de duplicidade.                                                                                                          | Alta       | Planejado |
| RF015 | O sistema deve permitir o cadastro de corredores vinculados a uma equipe já existente no sistema.                                                                                                                                                                | Alta       | Planejado |
| RF016 | O sistema deve validar que cada equipe possui exatamente 16 corredores cadastrados antes de permitir o início do evento, bloqueando o prosseguimento caso a condição não seja atendida.                                                                         | Alta       | Planejado |
| RF017 | O sistema deve permitir o registro do local ou região referente à etapa do evento.                                                                                                                                                                              | Baixa      | Planejado |
| RF018 | O sistema deve permitir a seleção da esteira na qual o corredor iniciará sua atividade.                                                                                                                                                                         | Alta       | Planejado |
...
| RF022 | O sistema deve permitir a edição retroativa de registros, com geração automática de log de auditoria contendo a identificação de quem realizou a alteração.                                                                                                     | Alta       | Planejado |
| RF023 | O sistema deve identificar e sinalizar inconsistências nos dados, como km_final inferior ao km_inicial, intervalo entre checkpoints superior a 10 minutos e corredor com turnos simultâneos em aberto.                                                          | Média      | Planejado |
| RF024 | O sistema deve permitir a exportação dos dados em formato CSV, contendo os turnos e checkpoints registrados.                                                                                                                                                    | Média      | Planejado |
| RF025 | O sistema deve suportar o registro de checkpoints e turnos sem conexão com a internet, persistindo os dados localmente e sincronizando-os automaticamente ao restabelecer a conexão, sem permitir duplicidade de registros.                                     | Alta       | Planejado |

A estrutura de requisitos apresentada foi concebida para transformar a dinâmica complexa do evento Red Bull 24 Horas em um fluxo digital ágil, seguro e auditável. Com essa base consolidada, o projeto avança para a fase de implementação, na qual cada ID listado atuará como critério de aceitação, garantindo que a apuração final dos quilômetros seja 100% confiável, rastreável e transparente para ambas as equipes.
